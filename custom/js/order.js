...
$(document).ready(function() {
	...
	if(divRequest == 'add')  {
		...
		$("#createOrderForm").unbind('submit').bind('submit', function() {
			...
			if(address == "") {
				$("#address").after('<p class="text-danger"> The  field is required </p>');
	   	...
	   	for (var x = 0; x < quantity.length; x++) {       						
			...
			if(orderDate && clientName && clientContact && paid && discount && paymentType && paymentStatus) {
				if(validateProduct == true && validateQuantity == true) {
	            	...
	            	'<strong><i class="fa fa-ok"></i></strong> '+ response.messages +
	            	' <br /> <br /> <a type="button" autofocus="true" id="printorder"  onclick="printOrder('+response.order_id+')" class="btn btn-primary"> <i class="fa fa-print"></i> Print </a>'+
	            	'<a href="orders.php?o=add" id="neworder" class="btn btn-default" style="margin-left:10px;"> <i class="fa fa-plus"></i> Add New Order </a>'+
		...
		$("#editOrderForm").unbind('submit').bind('submit', function() {
	   	...
	   	var quantity = document.getElementsByName('qquantity[]');		   	
	   	var validateQuantity;
	   	for (var x = 0; x < quantity.length; x++) {       
	 			var quantityId = quantity[x].id;
		    if(quantity[x].value == ''){	    	
	...
	if(orderId) {		
		...
		$.ajax({
			...
			success:function(response) {
				...
				var mywindow = window.open('', 'Stock Management System', 'height=600,width=600');
...
function pprintOrder(orderId = null) {
	if(orderId) {		
			
		$.ajax({
			url: 'php_action/printOrder1.php',
			type: 'post',
			data: {orderId: orderId},
			dataType: 'text',
			success:function(response) {
		...
		success:function(response) {
			...
			var tr = '<tr id="row'+count+'" class="'+arrayNumber+'">'+			  				
				...
				'<td style="padding-left:20px;"">'+
					...
					'<div class="input-group"><span class="input-group-addon">Purchase</span><input type="text" readonly name="purchase_rateValue[]" id="purchase_rateValue'+count+'" autocomplete="off" class="form-control" /></div>'+
					'<div class="input-group"><span class="input-group-addon">Stock</span><input type="text" readonly name="stock[]" id="stock'+count+'" autocomplete="off" class="form-control" /></div>'+
				'</td>'+
				'<td style="padding-left:20px;">'+
					'<div class="form-group">'+
				...
				'<td style="padding-left:20px;">'+
					'<div class="form-group">'+
					'<input type="number" name="pdiscount[]"  id="pdiscount'+count+'" value="0" onkeyup="getTotal('+count+')" autocomplete="off" value="0"  class="form-control" />'+
				'</td>'+
				'<td style="padding-left:20px;">'+
		...
		if(productId == "") {
			...
			$.ajax({
				...
				success:function(response) {
					...
					$("#f_Text"+row).val(response.product_name);
					$("#rate"+row).val(response.rate);
					$("#rateValue"+row).val(response.rate);
					$("#purchase_rateValue"+row).val(response.purchase);
...
function getTotal(row = null) {
	if(row) {
		var total = Number($("#rate"+row).val()) * Number($("#quantity"+row).val()) - (Number($("#rate"+row).val()) / 100 * Number($("#pdiscount"+row).val()));
		total = total.toFixed(2);
		$("#total"+row).val(total);
		$("#totalValue"+row).val(total);
		$("#rateValue"+row).val($("#rate"+row).val());
...
function subAmount() {
	...
	// sub total
	$("#subTotal").val(Number(totalSubAmount));
	$("#subTotalValue").val(Number(totalSubAmount));
	$("#show_current_amount").html('<label>Current Amount: '+previous_balanceValue+'+'+totalSubAmount+'='+(Number(previous_balanceValue)+Number(totalSubAmount))+'</label>');
