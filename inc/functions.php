...
<?php 
  function getSelectTagEnDis($data,$text){
    if (isset($data)) {
      # code...
      echo "<option value='".$data."'>".getLevel($data)."</option>";
    }else{
      echo "<option value=''>".$text."</option>";
...
<?php 
  function setColor($dbc, $tbl, $colName, $checkID, $fetchThis){
    ...
    echo ($id > 1) ? "text-success" : "text-danger";
  }
...
function countrySelector($defaultCountry = "", $id = "", $name = "", $classes = ""){
  ...
  foreach($countryArray as $code => $country){
    ...
    $output .= "<option value='".$countryName."' ".(($code==strtoupper($defaultCountry))?"selected":"").">".$code." - ".$countryName." (+".$country["code"].")</option>";
  }
...
function countryCurrency($id = "", $name = "", $classes = ""){
