  ...
  $fetchTransaction = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM transactions WHERE customer_id='$clientName' ORDER BY transaction_id DESC LIMIT 1"));
	...
	for($x = 0; $x < count($_POST['productName']); $x++) {		
		...
		while ($updateProductQuantityResult = $updateProductQuantityData->fetch_row()) {
			...
			$orderItemTableSql = "SELECT purchase_item.quantity FROM purchase_item WHERE purchase_item.purchase_id = {$orderId}";
	...
	for($x = 0; $x < count($_POST['productName']); $x++) {			
		$removeOrderSql = "DELETE FROM purchase_item WHERE purchase_id = {$orderId}";
		...
		for($x = 0; $x < count($_POST['productName']); $x++) {			
			...
			while ($updateProductQuantityResult = $updateProductQuantityData->fetch_row()) {
				$updateQuantity[$x] = $updateProductQuantityResult[0] - $_POST['quantity'][$x];							
					...
					// add into order_item
				$orderItemSql = "INSERT INTO purchase_item (purchase_id, product_id, quantity, rate, total, order_item_status) 
	...
	echo header('location:../editpurchase.php');
